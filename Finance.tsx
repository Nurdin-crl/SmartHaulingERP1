      ...
      <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            >
              <ICONS.Invoice className="w-4 h-4 text-blue-600" /> {isGeneratingPdf ? 'Processing...' : 'Cetak PDF'}
            </button>
            <button 
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh] text-slate-900 border border-slate-200">
             <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-6">
                <div>
                   <h4 className="text-2xl font-black uppercase tracking-tighter text-slate-950 italic">Entry Jurnal Manual</h4>
                   <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest mt-1">Sistem Double-Entry Enterprise</p>
                </div>
                <button onClick={() => setShowManualModal(false)} className="p-3 bg-slate-50 rounded-2xl text-slate-400 hover:text-red-500 transition-all"><ICONS.Plus className="w-6 h-6 rotate-45" /></button>
             </div>
             <form onSubmit={handlePostJournal} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="date" value={journalForm.date} onChange={e => setJournalForm({...journalForm, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-2 focus:ring-blue-500" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Nominal (IDR)</label>
                    <input type="number" placeholder="0" value={journalForm.amount || ''} onChange={e => setJournalForm({...journalForm, amount: Number(e.target.value)})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-2 focus:ring-blue-500" required />
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <select value={journalForm.category} onChange={e => setJournalForm({...journalForm, category: e.target.value as FinancialCategory})} className={`w-full bg-slate-50 border rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-2 transition-all ${journalForm.flowType === 'IN' ? 'border-emerald-200 focus:ring-emerald-500' : 'border-red-200 focus:ring-red-500'}`}>{filteredCategories.map(cat => <option key={cat.value} value={cat.value}>{cat.label}</option>)}</select>
                </div>
                <div className="space-y-1">
                  ...
                  <input type="text" placeholder="E.G. PENDAPATAN JASA" value={journalForm.accountId} onChange={e => setJournalForm({...journalForm, accountId: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black uppercase outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Deskripsi Lengkap</label>
                  <textarea placeholder="Deskripsi transaksi..." value={journalForm.description} onChange={e => setJournalForm({...journalForm, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black h-20 outline-none focus:ring-2 focus:ring-blue-500" required />
                </div>
                <button type="submit" className={`w-full py-5 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-3xl shadow-xl transition-all ${journalForm.flowType === 'IN' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}>Posting Jurnal & Update Saldo Kas</button>
             </form>
          </div>
        </div>
      )}
